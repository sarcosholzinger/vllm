...
class EngineArgs:
    ...
    def add_cli_args(parser: FlexibleArgumentParser) -> FlexibleArgumentParser:
        ...
        parser.add_argument(
            ...
            type=lambda configs: {
                str(key): value
                for key, value in (config.split(":") for config in configs.split(","))
            },
            default=None,
    ...
    def create_engine_config(self) -> EngineConfig:
        ...
        assert self.cpu_offload_gb >= 0, (
            "CPU offload space must be non-negative" f", but got {self.cpu_offload_gb}"
        )
        ...
        if self.num_scheduler_steps > 1:
            ...
            if self.enable_chunked_prefill and self.enable_prefix_caching:
                raise ValueError(
                    "Multi-Step is not supported with "
        ...
        num_lookahead_slots = max(
            self.num_lookahead_slots, self.num_scheduler_steps - 1
        )
        num_lookahead_slots = (
            num_lookahead_slots
